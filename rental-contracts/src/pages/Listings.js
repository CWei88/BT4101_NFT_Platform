...
const Listings=()=>{
  ...
  async function getListings(listings,rented){
  ...
  async function viewOwnedListings() {
    ...
    var data = [];
    try {
      await getListings(listings,false).then(res=>{
        data=res
      });
      console.log(data)
      setItemsList(data);
  ...
  if (userAuthenticated) {   
       ...
       <Grid item key={item[1]} xs={4}>
        <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column'}}>
          ...
          <Typography fontWeight='bold' fontFamily='BlinkMacSystemFont' paddingLeft={8}>
            {item[4]}
          </Typography>
